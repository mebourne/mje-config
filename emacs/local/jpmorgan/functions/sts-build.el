...
(defun sts-build (command)
  "Build the current STS source tree, and send the output to a compile
window.

It will run the command in the src directory of the current component,
...
The ClearCase view of the current file will be set automatically."
  ...
  (let ((view (clearcase-fprop-viewtag default-directory)))
    (if (not (and view
		  (string-match "^\\(.*/vobs/odyssey/[^/]*/\\)" default-directory)))
	(error "Not in an STS source tree"))
