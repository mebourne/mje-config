...
;; Revised:       Sun Feb 04 2001 10:52:24 (PST)
...
;; XEmacs causes an error when encoutering unknown keywords in the
;; menu definition.  Specifically the :help keyword is new in Emacs 21
;; and causes error for the XEmacs function `check-munu-syntax'.  IMHO
  ...
  (let* ((table-inhibit-advice t)
	...
	(save-excursion
	  ...
	  (let* ((border (format "[%c%c%c]"
	    ...
	    (while (re-search-forward border3 (point-max) t)
...
and any undo operation of table exhibit some known stange problems,
...
Flyspell minor mode is know to be incompatible with the table.
